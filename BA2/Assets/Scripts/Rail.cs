...
public class Rail : MonoBehaviour {
    ...
    public Vector3 CharacterPos(Vector3 pos)
    {
        int closestNodeIndex = GetClosestNode(pos);

        if(closestNodeIndex == 0)
        {
            return CharacterOnSegment(nodes[0], nodes[1], pos);
        }

        else if (closestNodeIndex == nodeCount -1)
        {
            return CharacterOnSegment(nodes[nodeCount - 1], nodes[nodeCount - 2], pos);
        }

        else
        {
            Vector3 leftSeg = CharacterOnSegment(nodes[closestNodeIndex - 1], nodes[closestNodeIndex], pos);
            Vector3 rightSeg = CharacterOnSegment(nodes[closestNodeIndex + 1], nodes[closestNodeIndex], pos);
    ...
    private Vector3 CharacterOnSegment(Vector3 v1, Vector3 v2, Vector3 pos)
    {
